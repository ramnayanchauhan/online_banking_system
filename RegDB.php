...
if ((!($uname))||(substr($uname, 0, 1)==' ')||(!($uname==mysqli_escape_string($uname)))||(!($uname==trim($uname))))
{ $Loc=$Loc."unameset=1&"; $flag=1; }
if ((!($pwd))||(!($pwd==mysqli_escape_string($pwd))))
{ $Loc=$Loc."pwdset=1&"; $flag=1; }
if ((!($trpwd))||(!($trpwd==mysqli_escape_string($trpwd))))
...
{   header($Loc.'Fname='.$fname.'&Lname='.$lname.'&Email='.$email.'&Uname='.$uname.'&day='.$day.'&Mnum='.$mnum.'&month='.$month.'&year='.$year.'&gen='.$gen.'&mnum='.$mnum.'&error=2'); exit(0);}
    ...
    else 
    {
        ...
        mysqli_query($sqlquery) or die("Unable to Register. Please contact Administrator");
        $ree=mysqli_query("SELECT * FROM customers WHERE loginid='$uname'");
        ...
        mysqli_query($sql) or die("Unable to Register. Please contact Administrator");
