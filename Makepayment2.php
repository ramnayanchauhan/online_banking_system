...
$payacno= $_POST['ac_no'];
$result1=mysqli_query($db,"SELECT * FROM registered_payee WHERE slno='$payto'");
if(mysqli_num_rows($result1) == 1)
{$arrpayment = mysqli_fetch_array($result1); $payeetype='ext';}
else
{
    $result1= mysqli_query($db,"SELECT * FROM customers WHERE customerid='$_POST[payto]'");
    $arrpayment1 = mysqli_fetch_array($result1);
    $payeetype='int';
    $arrpayment['payeename'] = $arrpayment1['firstname']." ".$arrpayment1['lastname'];
    $arrpayment['bankname'] = "Bank of RRACH City";
    $res1=mysqli_query($db,"SELECT * FROM accounts WHERE customerid='$_POST[payto]'");
...
$dt = date("Y-m-d h:i:s");
$custid=mysqli_real_escape_string($_SESSION['customerid']);
$resultpass=mysqli_query($db,"SELECT * FROM customers WHERE customerid='$custid'");
...
{         
	...
	{
            $rr=mysqli_query($db,"SELECT * FROM accounts WHERE customerid ='".$_SESSION['customerid']."'");
		...
		$amount=$_POST['payamt'];
                ...
                if (isset($_POST['payeetype']))
                {
                    ...
                    {     mysqli_query($db,"UPDATE accounts SET accountbalance = accountbalance+$amount WHERE customerid ='".$_POST[payto]."'") or die(mysqli_error ());
                    }
                }
                ...
                mysqli_query($db,"UPDATE accounts SET accountbalance = accountbalance-$amount WHERE customerid ='".$_SESSION['customerid']."'");
...
$custid=  mysqli_real_escape_string($_SESSION['customerid']);
$acc= mysqli_query($db,"SELECT * FROM accounts WHERE customerid='$custid'");
...
<div id="templatemo_main">
    <div id="sidecon">
        <form id="form1" name="form1" method="post" action="Makepayment3.php">
