...
$results = mysqli_query($db,"SELECT * FROM accounts where customerid='".$_SESSION['customerid']."'");
        ...
        if (isset($_REQUEST['numtran']))
        {
            ...
            while(($arrvar = mysqli_fetch_array($result))&&($count <= $_REQUEST['numtran'] ))
            {
                              ...
                              {echo "<td> </td> <td>$arrvar[amount]</td>";
                                  $r=  mysqli_query($db,"SELECT * FROM customers WHERE customerid='".$arrvar['payeeid']."'");
                                     $rarry= mysqli_fetch_array($r);
