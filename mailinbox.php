...
{
	mysqli_query($db,"DELETE FROM mail where mailid='$_GET[mailid]'");
	$recres = "Mail deleted Successfully...";
}
if(isset($_SESSION['adminid']))
$result = mysqli_query($db,"SELECT * FROM mail where reciverid='$_SESSION[adminid]'");
else if(isset($_SESSION['customerid']))
    $result= mysqli_query($db,"SELECT * FROM mail where reciverid='$_SESSION[customerid]'");
?>
<html>
<head>
                ...
                echo " </tr>";
         		while($row = mysqli_fetch_array($result))
  				{ $rid=$row['senderid'];
                                  if (!($rid=='admin'))
                                  {
                                      $rres=  mysqli_query($db,"SELECT * FROM customers WHERE customerid='".$rid."'");
